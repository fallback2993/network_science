...
def main():
    ...
    def run_experiment(algs, filename):

        collected_data = []
        iterations = list(range(0, 1))
        node_sizes = [250]
        mus = np.arange(0.1, 0.8, 0.1)
        configuration_set = itertools.product(*[iterations, algs.items(), node_sizes, mus])
        ...
        with mp.Pool(cpu_cnt) as pool:
            ...
            parallel_execution_data = pool.map(compute_experiment, configuration_set)
    ...
    # run_experiment(algorithms_for_experiment,'data-test.csv')    
...
# %%
    ...
    name = f"louvain"
    algorithms_for_experiment = {
        louvain_algorithm.run:name,
    }
    run_experiment(algorithms_for_experiment,f'data-{name}.csv')
    
    name = f"coverage"
    algorithms_for_experiment = {
        coverage_algorithm.run:name,
    }
    run_experiment(algorithms_for_experiment,f'data-{name}.csv')

    name = f"map-equation"
    algorithms_for_experiment = {
        infomap_algorithm.run:name,
    }
    run_experiment(algorithms_for_experiment,f'data-{name}.csv')
